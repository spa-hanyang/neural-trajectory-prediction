...
class lstm_bias_initializer(tf.keras.initializers.Initializer):
...
class lstm_kernel_initializer(tf.keras.initializers.Initializer):
  """Initializer that can be used with "tf.contrib.cudnn_rnn.CudnnLSTM"
...
def list_l2(list_y_true, list_y_pred, list_batch_size, new_scope=True):
  ...
  for gpu_idx, (y_true, y_pred, batch_size) in enumerate(zip(list_y_true, list_y_pred, list_batch_size)):
    with tf.device(tf.DeviceSpec(device_type="GPU", device_index=gpu_idx)), tf.name_scope("tower_{:d}".format(gpu_idx) if new_scope else tf.get_default_graph().get_name_scope() + "/tower_{:d}/".format(gpu_idx)):
      list_output.append(tf.reduce_sum(0.5 * tf.square(y_true - y_pred)) / tf.cast(batch_size, tf.float32))

  return list_output

def list_weighted_smooth_l1(list_y_true, list_y_pred, list_batch_size, new_scope=True):
  assert type(list_y_true) == list
  assert type(list_y_pred) == list
  list_output = []
  ...
  for gpu_idx, (y_true, y_pred, batch_size) in enumerate(zip(list_y_true, list_y_pred, list_batch_size)):
    with tf.device(tf.DeviceSpec(device_type="GPU", device_index=gpu_idx)), tf.name_scope("tower_{:d}".format(gpu_idx) if new_scope else tf.get_default_graph().get_name_scope() + "/tower_{:d}/".format(gpu_idx)):
      ...
      loss = tf.where(l1_part < 1,
                      x=l2_part,
                      y=l1_part)
      list_output.append(tf.reduce_sum(loss) / tf.cast(batch_size, tf.float32))
...
def list_cudnnlstm(list_input,
  ...
  for gpu_idx, (inputs, initial_states) in enumerate(zip(list_input, list_initial_state)):
    with tf.device(tf.DeviceSpec(device_type="GPU", device_index=gpu_idx)), tf.variable_scope("tower_{:d}".format(gpu_idx)):
      lstm = tf.contrib.cudnn_rnn.CudnnLSTM(num_layers, num_units, kernel_initializer=lstm_kernel_initializer(seed=seed), bias_initializer=lstm_bias_initializer(), seed=seed)
