  ...
  def _build_merge(self, hparams, final_state, global_feature, is_training):
    with tf.variable_scope('feature_merge'):
      with tf.variable_scope("fc1"):
        net = layers.list_dense_with_bn(global_feature, 512, is_training, self.bn_decay, seed=self.random_seed)
      
      with tf.variable_scope("fc2"):
        net = layers.list_dense_with_bn(net, 256, is_training, self.bn_decay, seed=self.random_seed)
      
      with tf.variable_scope("fc3"):
        net = layers.list_dense_with_bn(net, 64, is_training, self.bn_decay, seed=self.random_seed)
      
      with tf.name_scope("concat"):
        net = layers.list_concat(final_state, net, axis=-1)
